...
: QMainWindow(parent), qnode(argc, argv)
{
  ...
  headers << "Joint 1" << "Joint 2" << "Joint 3" << "Joint 4"
          << "Joint 5" << "Joint 6" << "Gripper" << "Status";
...
void MainWindow::on_btn_gripper_close_clicked(void)
{
  std::thread(
    [this]()
    {
      std::vector<double> joint_angle = {1.1};
...
void MainWindow::on_btn_save_pose_clicked(void)
{
  ...
  if (file.is_open()) {
    ...
    tableWidget->setItem(row, 7, statusItem);
  } else {
    ...
    [this, repeatCount]()
    {
      ...
      for (int repeat = 0; repeat < repeatCount; ++repeat) {
        if (qnode.isStopRequested()) {
          QMetaObject::invokeMethod(
            this, [this]() {
              writeLog("Play operation stopped by user.");
            }, Qt::QueuedConnection);
          break;
        }
        ...
        for (int row = 0; row < rowCount; ++row) {
          ...
          QMetaObject::invokeMethod(
            this, [this, row]() {
              tableWidget->item(row, 7)->setText("Pending");
            }, Qt::QueuedConnection);
        }
            ...
            if (joint_angle.size() == 7) {
              ...
              QMetaObject::invokeMethod(
                this, [this, row]() {
                  ...
                  tableWidget->item(row, 7)->setText("Executing...");
                }, Qt::QueuedConnection);

              if (!qnode.setJointSpacePath(joint_angle)) {
                QMetaObject::invokeMethod(
                  this, [this]() {
                    writeLog("[ERR!!] Failed to send joint space path service");
                  }, Qt::QueuedConnection);
                break;
              }

              if (std::abs(current_gripper_value - previous_gripper_value) > 0.1) {
              ...
              QMetaObject::invokeMethod(
                this, [this, row]() {
                  ...
                  tableWidget->item(row, 7)->setText("Done");
                }, Qt::QueuedConnection);
...
void MainWindow::on_btn_stop_clicked(void)
{
  ...
  for (int row = 0; row < rowCount; ++row) {
    ...
    tableWidget->item(row, 7)->setText("Stopped");
  }
